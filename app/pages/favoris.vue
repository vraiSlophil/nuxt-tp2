  ...
  <section class="space-y-4">
    ...
    <div class="card border-2 border-base-300 bg-base-100">
      <div class="card-body gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="card-title text-2xl">Favoris de {{ user?.login }}</h1>
          <p class="text-sm text-base-content/70">Cette liste est maintenant filtrée par utilisateur connecté.</p>
        </div>

        <NuxtLink to="/profil" class="btn btn-outline btn-sm">
    ...
    <div v-else-if="favorites.length === 0" class="alert">
      <span>Aucune bière en favori pour le moment.</span>
    </div>
    ...
    <ul v-else class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3" aria-label="Liste des favoris">
      <li v-for="favorite in favorites" :key="`${favorite.favoriteType}-${favorite.id}`"
        ...
        <figure class="relative h-52 w-full bg-base-200">
          ...
          <span class="badge absolute bottom-2 left-2">{{ favorite.favoriteType }}</span>
        </figure>
        ...
        <div class="card-body p-4">
          ...
          <div class="flex flex-wrap gap-2 text-sm">
            ...
            <span class="badge badge-outline">{{ formatFavoritedAt(favorite.favoritedAt) }}</span>
          </div>
